...
def createHistogram(noerror=False):
    ...
    return histogram.histogram(
        name="I(E, TubeId)", data=data, errors=errors, axes=[axisE, axisTubeId]
    )
...
class Histogram_TestCase(TestCase):
    ...
    def testSetItem(self):
        ...
        h = histogram("h", [("a", [1, 2, 3]), ("b", [4, 5])])
        h[1, 5] = 1, 1
        self.assertVectorAlmostEqual(h.data().storage().asList(), [0, 1, 0, 0, 0, 0])
        self.assertVectorAlmostEqual(h.errors().storage().asList(), [0, 1, 0, 0, 0, 0])
    ...
    def testClear(self):
        ...
        self.assertVectorAlmostEqual(h[1, 4], (1, 1))
        h.clear()
        self.assertVectorAlmostEqual(h.data().storage().asList(), [0, 0, 0, 0, 0, 0])
        self.assertVectorAlmostEqual(h.errors().storage().asList(), [0, 0, 0, 0, 0, 0])
        return
    ...
    def testSetSlice(self):
        ...
        histogram[0.5, ()][(1, 3)] = 0.0, 0.0

        # setitem using dictionary
        histogram[{"E": 1.5, "tubeId": 3}] = 3, 3
        self.assertVectorAlmostEqual(histogram[{"E": 1.5, "tubeId": 3}], (3, 3))

        histogram[{"E": 1.5}][{"tubeId": 3}] = 3, 3
        self.assertVectorAlmostEqual(histogram[{"E": 1.5, "tubeId": 3}], (3, 3))

        #
        histogram[{"E": 1.5}] /= 3.0, 3.0
        histogram[{"E": 1.5}] /= 3.0, 3.0
        return
    ...
    def test__idiv__(self):
        ...
        h /= (2.0, 0.0)
        self.assertVectorEqual(h[0.5, 1], (0, 0))
        self.assertVectorEqual(h[0.5, 3], (0.5, 1.0 / 4))
        return
    ...
    def test__add__(self):
        ...
        h3 = h1 + h2
        for x in range(10):
            self.assertAlmostEqual(h3[x][0], 3)
        return
    ...
    def test_axisFromId(self):
        ...
        x = axis("x", arange(1.0, 10.0, 1.0))
        y = axis("y", arange(-1.0, 1.0, 0.1))
        h = histogram("h", (x, y))
    ...
    def test_getattribute(self):
        ...
        t1 = h.I
        t2 = h.data().storage().asNumarray()
        t1.shape = t2.shape = (-1,)
        self.assertVectorEqual(t1, t2)

        t1 = h.E2
        t2 = h.errors().storage().asNumarray()
        t1.shape = t2.shape = (-1,)
        self.assertVectorEqual(t1, t2)
...
"""
        ...
        try:
            ...
            raise RuntimeError("failed to load histogram from %s" % (outh5,))
