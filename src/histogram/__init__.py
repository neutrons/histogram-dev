...
## data, and we have an abstract interface
...
def axis(name, centers=None, unit=None, boundaries=None, attributes=None):
    ...
    if centers is not None and len(centers) < 1:
        raise ValueError("Invalid axis %s" % (centers,))
    ...
    if centers is None:
        return paxis(name, unit, boundaries=boundaries, attributes=attributes)
...
def histogram(
    name, axes, data=None, errors=None, unit="1", data_type="double", fromfunction=None
):
    ...
    """
    h = makeHistogram(name, axes, data, errors, unit=unit, data_type=data_type)
    ...
    if len(slices) == 1:
        slices = slices[0]  # dim=1 is a special case
    if isinstance(fromfunction, tuple) or isinstance(fromfunction, list):
        ...
        datasets = [datasetFromFunction(f, axes) for f in fromfunction]
        pass
...
def plot(h, min=None, max=None, output="window", interactive=False, **kwds):
    """plot a histogram
...
def datasetFromFunction(func, axes, *args, **kwds):
    ...
    try:
        return applyFunction_fast(func, xs, *args, **kwds)
    except:
        import traceback
...
def histogramContainer(input):
    ...
    assert isinstance(input, DetHistCollection), "%s is not a histogram collection" % (
        input,
    )
    return input
...
def createContinuousAxis(
    name, unit, centers=None, boundaries=None, centersCreationArgs=None, attributes=None
):
    ...
    """
    if (
        centers is not None or centersCreationArgs is not None
    ...
    from .EvenlyContinuousAxisMapper import (
        EvenlyContinuousAxisMapper as AxisMapper,
        NotEvenlySpaced,
    )
...
def boundariesFromCenters(centers):
    ...
    for i in range(1, len(centers) - 1):
        ...
        continue
    try:
        return _boundariesFromEvenlySpacedCenters(centers)
    except ArrayNotEvenlySpaced:
        return _boundariesFromCenters(centers)
...
def _boundariesFromCenters(centers):
    ...
    ret[1:-1] = (c1 + c2) / 2.0
    ret[0] = c[0] - (c[1] - c[0]) / 2.0
    ret[-1] = c[-1] + (c[-1] - c[-2]) / 2.0
    return list(ret)


class ArrayNotEvenlySpaced(Exception):
    pass


class BinsOverlapped(Exception):
    pass
...
def makeHistogram(name, axes, data, errs, unit="1", data_type="double"):
    ...
    for axis_params in axes:
        if isinstance(axis_params, Axis):
            _axis = axis_params
        else:
            _axis = axis(*axis_params)
        _axes.append(_axis)
        continue
    ...
    if data is None:
        data = createDataset("data", unit, shape=shape, data_type=data_type)
    from .DatasetBase import DatasetBase

    if isinstance(data, DatasetBase):
        dataDS = data
    else:
        dataDS = createDataset("data", unit, data=data, data_type=data_type)

    if errs is None:
        errs = createDataset(
            "errors", unit * unit, shape=shape, data_type=dataDS.typecode()
        )
    if isinstance(errs, DatasetBase):
        errsDS = errs
    else:
        errsDS = createDataset("errors", unit**2, data=errs, data_type=data_type)

    from .Histogram import Histogram

    h = Histogram(name=name, unit=unit, data=dataDS, errors=errsDS, axes=_axes)
