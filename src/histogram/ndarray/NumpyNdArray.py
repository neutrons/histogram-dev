...
for n, v in list(types.items()):
    exec("AK_%s = %d" % (n.upper().replace(" ", "_"), v))
...
class NdArray(AbstractNdArray):
    def __init__(self, datatype, arg2, initVal=None, **kwds):
        ...
        if isinstance(datatype, int):
            t = numpytypecode_from_aktypecode(datatype)
        elif datatype in typename2npytypecode_dict:
            t = numpytypecode_from_typename(datatype)
        else:
        ...
        try:
            len(arg2)  # test if a list-like object
        except:
    ...
    def __rtruediv__(self, other):
        ...
        if isNdArray(other):
            ...
            raise NotImplementedError(
                "%s / %s" % (other.__class__.__name__, self.__class__.__name__)
            )
    ...
    def __getitem__(self, s):
        ...
        if isinstance(s, int):
            ...
            raise NotImplementedError(
                "Don't know how to get element indexed by %s" % (s,)
            )
...
def getAKTypecode(arr):
    ...
    try:
        return getNumpyArray_aktypecode(arr)
    except:
        try:
...
def getNumpyArray_aktypecode(arr):
    ...
    if name == "int32":
        ...
        raise NotImplementedError(
            "don't know how to determine type of %s:%s" % (t, arr)
        )
    raise "Should not reach here"
