...
class Bin:
    ...
    def getMidpoint(self):
        return self.leftSide + self.spacing / 2.0
    ...
    def value(self):
        ...
        for ind, xPt, yPt in zip(list(range(self.numPts)), self.xPts, self.yPts):
            ...
            totalVol += boxVol
        totalHeight = 1.0 * totalVol / self.spacing
        return totalHeight
...
class SimpleRebin:
    ...
    def __init__(
        self, xDataP, yDataP, newBins=None, binValue="countPoints", format="columns"
    ):
        ...
        if type(newBins) is type(1):
            ...
            leftOverPts, numXdataInBin = math.modf(len(xData) * 1.0 / len(newBins))
            ...
            for xPt, yPt in zip(xData, yData):
                ...
                if binValue == "countPoints":
                    ...
                    currentBin.value = (numPointsInBin * currentBin.value + yPt) / (
                        numPointsInBin + 1.0
                    )
        else:
