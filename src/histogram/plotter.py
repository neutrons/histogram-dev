...
class HistogramPlotter:
    def interactive(self, b):
        ...
        raise NotImplementedError(
            "%s must override interactive" % (self.__class__.__name__)
        )
...
def axis_label(axis):
    label = axis.name()
    if len(str(axis.unit())) > 0:
        label = label + "(%s)" % axis.unit()
    return label
...
def resample(hist1, size):
    ...
    step = 1.0 * (back - front) / size
    newxbb = H.arange(front, back + step / 2.0, step)
    ...
    for x in newxc[1:-1]:
        newhist[x] = hist1[(x - step / 2.0, x + step / 2.0)].sum()
        continue

    newhist[newxc[0]] = hist1[(newxbb[0], newxc[0] + step / 2.0)].sum()
    newhist[newxc[-1]] = hist1[
        (newxc[-1] - step / 2.0, newxbb[-1] - step * 1.0e-10)
    ].sum()
