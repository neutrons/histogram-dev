...
import journal
debug = journal.debug("HistPanel")
warning = journal.warning("HistPanel")
...
class HistPanel(wx.Panel):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.Bind(wx.EVT_BUTTON, self.OnPlotButton, self.plotButton)

        sizer = wx.BoxSizer( wx.VERTICAL )

        buttonsSizer = wx.BoxSizer(wx.HORIZONTAL)
        buttonsSizer.Add( self.plotButton, 0, wx.ALIGN_CENTER)
        sizer.Add( buttonsSizer, 0, wx.ALL, 20 )
    ...
    def showHist(self, histIdentifier):
        ...
        for axisName in hist.axisNameList():
            ...
            axes += "  Axis %s: %s\n" % (
                axisName, _str(axis.storage().asNumarray() ) )
            continue
    ...
    def OnPlotButton(self, event):
        ...
        if dim > 2:
            ...
            return 
        if dim == 1: plot1d( hist ); return
        if dim == 2: plot2d( hist ); return
        raise


    pass #end of HistPanel
