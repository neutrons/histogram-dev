...
class Axis(Dataset):
    ...
    def __init__(
        ...
        if length == 0 and storage is not None:
            length = storage.size() - 1
    ...
    def __getitem__(self, s):
        ...
        """
        if not isSlicingInfo(s):
            return Dataset.__getitem__(self, s)
        slicingInfo = s
    ...
    def __neg__(self):
        return self._overloaded_operator("__neg__")

    def __add__(self, other):
        return self._overloaded_operator("__add__", other)

    __radd__ = __add__

    def __sub__(self, other):
        return self._overloaded_operator("__sub__", other)

    def __rsub__(self, other):
        return self._overloaded_operator("__rsub__", other)

    def __mul__(self, other):
        return self._overloaded_operator("__mul__", other)

    __rmul__ = __mul__

    def __div__(self, other):
        return self._overloaded_operator("__div__", other)

    def __rdiv__(self, other):
        return self._overloaded_operator("__rdiv__", other)

    def __iadd__(self, other):
        return self._overloaded_operator("__iadd__", other)

    def __isub__(self, other):
        return self._overloaded_operator("__isub__", other)

    def __imul__(self, other):
        return self._overloaded_operator("__imul__", other)

    def __idiv__(self, other):
        return self._overloaded_operator("__idiv__", other)
    ...
    def slicingInfo2IndexSlice(self, slicingInfo):
        ...
        if isDimensional(unit):
            if isDimensional(start):
                start = start / unit
            if isDimensional(end):
                end = end / unit
        ...
        if not isNumber(start) or not isNumber(end):
            raise RuntimeError(
                "At this point, start and end should all be numbers: start=%s(%s), end=%s(%s)"
                % (start, type(start), end, type(end))
            )
        # slice. +1 is due to the difference of bin boundaries and bin centers
        s = (self.cellIndexFromValue(start), self.cellIndexFromValue(end) + 1)
        return s
