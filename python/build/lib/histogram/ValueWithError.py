...
class ValueWithError(object):
    ...
    def asTuple(self):
        return self._value, self._error2 or 0.0
...
def toVE(candidate):
    if isinstance(candidate, ValueWithError):
        return candidate
    ...
    if isNumber(candidate):
        return ValueWithError(candidate)
    raise NotImplementedError("Don't know how to convert {0!s}".format(candidate))
...
def isIterable(candidate):
    if isinstance(candidate, ListType):
        return True
    if isinstance(candidate, TupleType):
        return True
    if "__iter__" in dir(candidate):
        return True
    return False
...
def isNumber(candidate):
    numbertypes = [LongType, FloatType, IntType]

    for t in numbertypes:
        if isinstance(candidate, t):
            return True
        continue
    return False
