...
import sys
import numpy as np
from ..ndarray.NumpyNdArray import (
    NdArray,
    getNumpyArray_aktypecode as getDataType,
    arrayFromNumpyArray,
)
...
class Loader:
    ...
    def onHistogram(self, **kwds):
        ...
        if kwds:
            ...
            h = histogram(
                self.histogramName, axisList, data=data, errors=errors, unit=data.unit()
            )
    ...
    def onHistogramSlice(self, h5group, axes, **kwds):
        ...
        h = histogram(
            self.histogramName, axes, data=data, errors=errors, unit=data.unit()
        )
        return h
    ...
    def onUnit(self, unit):
        ...
        try:
            ...
            raise NotImplementedError("type: %s, str: %s" % (unit.__class__, str(unit)))
