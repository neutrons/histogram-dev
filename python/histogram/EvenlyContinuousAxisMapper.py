...
class EvenlyContinuousAxisMapper(AxisMapper):
    ...
    def __init__(
        ...
        if binBoundaries is not None:
            ...
            if len(bb) < 2:
                raise "Too few bin boundaries: %s" % len(bb)
            binSize = bb[1] - bb[0]
    ...
    def __call__(self, value):
        if self._positveStep:
            if value < self._minBB or value >= self._maxBB:
                raise IndexError(
                    "%s out of bounds (%s,%s)" % (value, self._minBB, self._maxBB)
                )
        else:
            if value > self._minBB or value <= self._maxBB:
                raise IndexError(
                    "%s out of bounds (%s,%s)" % (value, self._maxBB, self._minBB)
                )
...
class NotEvenlySpaced(Exception):
    pass
...
def assertEvenlySpaced(bb):
    ...
    for i in range(1, len(bb) - 1):
        d = bb[i + 1] - bb[i]
        if abs((d - d0) / d0) > epsilon:
            raise NotEvenlySpaced("%s is not a evenly spaced array" % (bb,))
        continue
    return
