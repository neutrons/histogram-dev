...
class Plotter1D(object):
    def interactive(self, b):
        ...
        raise NotImplementedError("%s must override plot" % (self.__class__.__name__))
...
class MplPlotter1D(MplPlotter, Plotter1D):
    ...
    def errorbar(self, x, y, yerr=None, **kwds):
        ...
        if self._usePylab:
            plot = self._engine.errorbar
        else:
            if figure is None:
                print("missing matplotlib! ")
                return
            else:
                plot = figure.gca().errorbar
            pass
        rt = plot(x, y, yerr=yerr, **kwds)
        if self._usePylab and not self._interactive:
            self._engine.show()
        return rt
    ...
    def _plot(self, x, y, *args, **kwds):
        ...
        if self._usePylab:
            plot = self._engine.plot
        else:
            if figure is None:
                print("missing matplotlib! ")
                return
            else:
                plot = figure.gca().plot
            pass
...
class Plotter2D(object):
    ...
    def contourPlot(self, x, y, z, min=None, max=None, nsteps=20):
        ...
        raise NotImplementedError(
            "%s must override contourPlot" % (self.__class__.__name__)
        )
...
class MplPlotter2D(MplPlotter, Plotter2D):
    ...
    def plot_(self, x, y, z, min=None, max=None, **kwds):
        ...
        if len(x) <= 1 or len(y) <= 1:
            raise ValueError(
                "The data you want to plot is really 1-D. Please make a create a 1D histogram by slicing and make a 1D plot"
            )
        ...
        if "aspect" not in kwds:
            kwds["aspect"] = "auto"
        if "extent" not in kwds:
            kwds["extent"] = extent
    ...
    def contourPlot_(self, x, y, z, min=None, max=None, nsteps=20):
        ...
        # color steps
        c = N.arange(min, max, 1.0 * (max - min) / nsteps)

        print("plot z in (%s, %s)" % (min, max))
        _clear(figure)
        if self._usePylab:
            self._engine.contourf(x, y, z, c)
        else:
            figure.gca().contourf(x, y, z, c)
        # self._engine.show()
        return
